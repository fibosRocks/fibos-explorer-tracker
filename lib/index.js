...
let block_caches = new cache.LRU({max: 20, ttl: 1000 * 60 * 60 * 24});
					...
					db.trans(() => {
						...
						function saveActions(m, p_id) {
							...
							if (_m.receipt.receiver == _m.act.account) {
								_p_id = db.driver.execQuerySync(`insert into fibos_actions(trx_id,global_sequence,contract_action,contract,action,rawData,parent_id,transaction_id) values(?,?,?,?,?,?,?,?)`, [_m.trx_id, _m.receipt.global_sequence, _m.act.account + "/" + _m.act.name, _m.act.account, _m.act.name, JSON.stringify(_m), p_id, transaction.id]).insertId;
								saveAccountActions(_m.receipt.receiver, _p_id, _m.receipt)
								// add auth account
								_m.act.authorization.forEach(auth => {
									if (auth.actor != _m.act.account) {
							...
							function saveAccountActions(account, action_id, receipt) {
								...
								if (!isNaN(action_id) && account) {
									// insert account action relationship
									let FibosAccountActions = db.models.fibos_account_actions;
									let faa = FibosAccountActions.oneSync({
										account, action_id
									})

									if (!faa) {
										FibosAccountActions.createSync({
											account, action_id, receipt: JSON.stringify(receipt)
										});
									}
								}
							}
					...
					db.trans(() => {
						...
						if (deal_block.length) {
							deal_block.forEach(bk => {
								...
								db.driver.execQuerySync(`update fibos_blocks set status = 'lightconfirm' where producer_block_id = ?`, [bk.id]);
